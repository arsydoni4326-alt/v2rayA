...
本地节点和各订阅节点标签页支持在独立筛选面板中按协议和加密方式筛选；筛选选项会根据对应节点动态生成，并在切换标签页时保留。TLS 已启用但加密为 `none` 或 `false` 的 VMess 节点会保留显示并标为不可选择。VLESS+TLS 始终允许（VLESS 协议加密始终为 `none`，TLS 提供传输安全）。“按延迟批量添加”仅在本地节点或单个订阅节点标签页可用。

实时流量拓扑会排除内部延迟探测使用的 `gstatic.com:443`，避免探测请求淹没画布。
