...
- **Server eligibility:** TLS-enabled VMess/VLESS entries with `none` or `false` encryption remain visible with a reason but cannot be selected or connected; latency bulk addition is available only from a server table
